...
public class New_moim_GridLayout_Adapter extends RecyclerView.Adapter<New_moim_GridLayout_Adapter.ViewHolder> {

    private Context mContext;
    private ArrayList<New_Moim_Data> New_Moim_DataList;


    // Provide a suitable constructor (depends on the kind of dataset)
    public New_moim_GridLayout_Adapter(Context mContext,ArrayList<New_Moim_Data> myDataset) {
        this.mContext = mContext;
        this.New_Moim_DataList = myDataset;
    }
    ...
    @Override
    public New_moim_GridLayout_Adapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        // create a new view
        ...
        New_moim_GridLayout_Adapter.ViewHolder vh = new New_moim_GridLayout_Adapter.ViewHolder(v);  // 이 view를 넣기위한 ViewHolder를 다음과 같이 선언한 후 넣는다.
        return vh;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, final int position) {
        ...
        // 안보임
        holder.M_no.setText(New_Moim_DataList.get(position).M_no);                  // 모임 번호
//        holder.M_Category.setText(New_Moim_DataList.get(position).Number);          // 카테고리 보이게 할 예정
        // 보임
        holder.Date.setText(New_Moim_DataList.get(position).Date);                  // 날짜
        holder.simple_address.setText(New_Moim_DataList.get(position).simple_address); // 간단 모임주소


        // 유료, 무료 설정
        if(New_Moim_DataList.get(position).Paid_or_Free.equals("유료")){ // 유료 경우
            holder.Paid_or_Free.setText(New_Moim_DataList.get(position).Paid_or_Free);  // 유료 or 무료
        ...
        // 이미지 파일 이름 가져오기
        String img_NAME = New_Moim_DataList.get(position).Img_Name;
...
//                Toast.makeText(view.getContext(),position+" 번 아이템 선택 됨.",Toast.LENGTH_SHORT).show();
                ...
                mIntent.putExtra("NUMBER",New_Moim_DataList.get(position).M_no );
